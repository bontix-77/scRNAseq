...
\-**Code** folder - containing all the algorithm each as module in its own sub-folder. here there is also a sub-directory called "python" containing a couple of scripts to download the GSM files from GEO and for the gene ID conversion for example from Ensebl ID to gene symbol.

...
annot <- SingleR::SingleR(
  test = HIV.sce,
  ref = humanRNA,
  labels = humanRNA$label.fine
)
head(annot)
...
clustAnnot <- SingleR::SingleR(
  test = avgExp,
  ref = humanRNA,
  labels = humanRNA$label.fine
) # Run SingleR on the averaged expression matrix
clustAnnot
...
clustLabels.vect <- clustLabels[match(
  HIV_pp_mks$seurat_clusters,
  names(clustLabels)
)] # match the cluster identities per cell in the Seurat data to the cluster labels
...
clustAnnotFig4 <- DimPlot(
  HIV_pp_mks,
  group.by = "humanRNA.fine.clust",
  label = T,
  repel = T,
  label.size = 2
)

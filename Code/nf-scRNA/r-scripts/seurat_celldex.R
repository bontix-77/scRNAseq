...
clustAnnotFig1 <- DimPlot(HIV_pp_mks, group.by = "seurat_clusters", label = T) + NoLegend()
# clustAnnotFig2 <- DimPlot(HIV_pp_mks, group.by = "cell_type", label = T) + NoLegend()
clustAnnotFig3 <- DimPlot(HIV_pp_mks, group.by = "humanRNASeq.fine",label= T) + NoLegend()
clustAnnotFig4 <- DimPlot(HIV_pp_mks, group.by = "humanRNA.fine.clust", label = T)

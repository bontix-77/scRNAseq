...
```{r  CellRanger data upload, dataset merging and Seurat object definition}
library(Seurat)
...
```
```{r Normalization, transformation  and dimension reduction}
# run sctransform
...
fig1 / (fig2 | fig3)
```
```{r pseudo bulk Differential Expression}
...
## after finding the markers associated with particular cluster we can annotate for the cell type.
